...
export function ActivityTile() {
  ...
  return (
    <section
      className="rounded-2xl bg-zinc-950/40 backdrop-blur-md border border-white/8 p-5 shadow-card flex flex-col justify-between h-full"
    ...
    >
      ...
      <div className="flex gap-3 items-end justify-start overflow-x-auto py-1 scrollbar-none">
        <div className="flex flex-col gap-0.75 text-[9px] font-bold text-gray-500 select-none shrink-0" aria-hidden="true">
          <span className="h-2.75 flex items-center">Mon</span>
          <span className="h-2.75 flex items-center">Tue</span>
          <span className="h-2.75 flex items-center">Wed</span>
          <span className="h-2.75 flex items-center">Thu</span>
          <span className="h-2.75 flex items-center">Fri</span>
          <span className="h-2.75 flex items-center">Sat</span>
          <span className="h-2.75 flex items-center">Sun</span>
        </div>

        <div className="flex-1 flex flex-col min-w-75 max-w-100">
          <div className="grid grid-cols-24 gap-0.75 text-[9px] font-bold text-gray-500 mb-1.5 select-none" aria-hidden="true">
          ...
          <div
            className="grid grid-cols-24 gap-0.75 select-none"
            role="img"
                ...
                {week.map((level, j) => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate="visible"
                      className={cn(
                        "w-2.75 h-2.75 rounded-[1px] transition-all duration-200 cursor-pointer",
                        getColor(level)
                      )}
                      role="img"
      ...
      <div
        className="flex justify-between items-center text-[9px] font-bold text-gray-500 mt-4 pt-3 border-t border-white/6"
        aria-hidden="true"
      >
        ...
        <div className="flex items-center gap-1.5 text-gray-500">
          ...
          <div className="flex gap-0.75 items-center">
            {[0, 1, 2, 3].map((level) => (
              <div
                key={level}
                className={cn("w-2.75 h-2.75 rounded-[1px]", getColor(level).split(' hover:')[0])}
                aria-label={getAccessibilityLabel(level)}
              />
            ))}
          </div>
